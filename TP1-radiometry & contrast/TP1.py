...
#%% charger une image 
im=skio.imread('Téléchargements/images/maison.tif')
...
print (r.shape)
#%% une image couleur 
im=skio.imread('Téléchargements/images/fleur.tif')
...
viewimage(imequal)
(histo,bins)=np.histogram(imequal.reshape((-1,)),np.arange(0,256)) #le reshape en inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
histo=histo/histo.sum()
histocum=histo.cumsum()
plt.plot(histocum)
plt.show()
...
#%%
im=skio.imread('Téléchargements/images/rayures.tif')
view_spectre(im,option=2,hamming=True)

#%% Ringing
im=skio.imread('Téléchargements/images/lena.tif')
lowim=filterlow(im)
gaussim=filtergauss(im)
viewimage(im)
view_spectre(im, option=2, hamming=False)
viewimage(lowim)
view_spectre(lowim, option=2, hamming=False)
viewimage(gaussim)
view_spectre(gaussim, option=2, hamming=False)

 #%%
iml=skio.imread('Téléchargements/images/masque_bas_centre.tif')
viewimage(iml)
img=skio.imread('Téléchargements/images/masque_gauss_centre.tif')
viewimage(img)
#%% FIN TP INTRODUCTION
